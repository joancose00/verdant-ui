...
export async function getCachedRatios(chain, limit = 100) {
  try {
    const ratios = await sql`
      SELECT address, total_deposits, total_withdrawals, ratio, total_sells, sell_ratio, total_miners, active_miners, last_calculated_at
      FROM address_ratios 
      WHERE chain = ${chain}
    ...
    return ratios.map(row => ({
      ...
      totalSells: parseFloat(row.total_sells || 0).toFixed(2),
      sellRatio: parseFloat(row.sell_ratio || 0).toFixed(4),
 ...
 */
export async function storeRatioData(chain, address, totalDeposits, totalWithdrawals, ratio, totalMiners = 0, activeMiners = 0, totalSells = 0, sellRatio = 0) {
  try {
    console.log(`💾 STORING RATIO DATA: ${address} -> deposits=${totalDeposits}, withdrawals=${totalWithdrawals}, ratio=${ratio}`);
    
    const result = await sql`
      INSERT INTO address_ratios (address, chain, total_deposits, total_withdrawals, ratio, total_sells, sell_ratio, total_miners, active_miners)
      VALUES (${address}, ${chain}, ${totalDeposits}, ${totalWithdrawals}, ${ratio}, ${totalSells}, ${sellRatio}, ${totalMiners}, ${activeMiners})
      ON CONFLICT (address, chain)
      DO UPDATE SET
